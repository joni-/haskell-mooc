...
instance Num RationalNumber where
  (RationalNumber x1 y1) + (RationalNumber x2 y2) = simplify (RationalNumber (x1*y2 + y1*x2) (y1*y2))
...
class Cycle a where
  ...
  stepMany n from = go n from where
    go 0 v = v
    go n v = go (n-1) (step v)

instance Cycle Color where
  step from = case from of
    Red   -> Green
    Green -> Blue
    Blue  -> Red
