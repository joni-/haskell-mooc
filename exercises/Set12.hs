...
count x = foldr (\e acc -> if x == e then acc + 1 else acc) 0
...
inBoth xs ys = foldr (\e acc -> if count e ys > 0 then e : acc else acc) [] xs
...
instance Foldable List where
  ...
  foldr f initialValue (LNode v Empty) = f v initialValue
  foldr f initialValue (LNode v rest) = f v (foldr f initialValue rest)
...
instance Foldable TwoList where
  ...
  foldr f initialValue (TwoNode x y TwoEmpty) = f x $ f y initialValue
...
instance Functor Tree where
  fmap f Leaf = Leaf
  fmap f (Node a xs ys) = Node (f a) (fmap f xs) (fmap f ys)

sumTree :: Monoid m => Tree m -> m
sumTree Leaf = mempty
sumTree (Node a left right) = mconcat [sumTree left, a, sumTree right]
