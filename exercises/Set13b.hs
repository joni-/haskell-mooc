...
ifM opBool opThen opElse = do
  boolValue <- opBool
  if boolValue then opThen else opElse
...
visit maze place = do
  ...
  if place `elem` visited
    then put visited
      ...
      modify (place :)
      mapM_ (visit maze) (nbs place)
  where
    nbs v = case lookup v maze of
      (Just nbs') -> nbs'
      Nothing -> []
...
path maze place1 place2 = place2 `elem` reachable
  where
    (_, reachable) = runState (visit maze place1) []
...
findSum2 ks ns = filter (\(_, _, x) -> x `elem` ns) triplets
  where
    triplets = do
      k1 <- ks
      k2 <- ks
...
instance Functor SL where
  ...
  fmap f s =
    SL
      ( \input ->
          let (value, state, msgs) = runSL s input
           in (f value, state, msgs)
      )
...
instance Monad SL where
  -- implement return and >>=
  return x = SL (\input -> (x, input, []))
  (>>=) a b =
    SL
      ( \input ->
          let (aValue, s, msg) = runSL a input
              (bValue, s', msg') = runSL (b aValue) s
           in (bValue, s', msg ++ msg')
      )
