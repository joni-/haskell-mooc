...
cull _ Empty          = Empty 
cull val (Node v l r) = if val == v then Empty else (Node v (cull val l) (cull val r))
...
isOrdered (Node v l r)  = allValues (<v) l && isOrdered l && allValues (>v) r && isOrdered r
...
search val tree = helper tree [] where
  helper Empty _            = Nothing
  helper (Node v l r) steps = if val == v
    then Just (reverse steps)
    else case (helper l (StepL:steps), helper r (StepR:steps)) of
      ((Just steps), _)  -> (Just steps)
      (_, (Just steps))  -> (Just steps)
      _                  -> Nothing
