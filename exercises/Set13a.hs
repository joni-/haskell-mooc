...
checkNumber (x, y) = if any isDigit (x ++ y) then Nothing else Just (x, y)
...
checkCapitals (for@(x : _), sur@(y : _)) = if isLower x || isLower y then Nothing else Just (for, sur)
...
safeIndex xs i
  ...
  | otherwise = case xs of
    (x : xs) -> if i == 0 then Just x else safeIndex xs (i - 1)
...
countAndLog p xs = Logger (map show matches) (length matches)
  where
    matches = filter p xs
...
balance accountName = BankOp f
  where
    f (Bank bank) = case Map.lookup accountName bank of
      Just balance -> (balance, Bank bank)
      Nothing -> (0, Bank bank)
...
incs = Map.fromList [('(', 1), (')', -1)]

lookupDefault :: Ord k => k -> p -> Map.Map k p -> p
lookupDefault k defaultValue m = case Map.lookup k m of
  Just v' -> v'
...
paren c = do
  value <- get
  let newValue = if value == (-1) then (-1) else value + lookupDefault c 0 incs
  put newValue
...
count x = do
  matches <- get
  let c = currentValue x matches
  put $ increment x matches
  where
    currentValue x matches = case lookup x matches of
      Just v -> v
      Nothing -> 0
    increment key [] = [(key, 1)]
    increment key ((k, currentValue) : xs) = if k == key then (k, currentValue + 1) : xs else (k, currentValue) : increment key xs
...
occurrences xs = do
  state <- get
  let newState = foldr (\x state' -> snd $ runState (count x) state') state xs
  put newState
  return $ length newState
