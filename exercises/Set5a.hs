...
rgb Green       = [0,1,0]
rgb Blue        = [0,0,1]
rgb (Invert c)  = [1-r, 1-g, 1-b] where
  (r:g:b:_) = rgb c
rgb (Mix c1 c2) = [(r1+r2)/2, (g1+g2)/2, (b1+b2)/2] where
  ((r1:g1:b1:_), (r2:g2:b2:_)) = (rgb c1, rgb c2)
------------------------------------------------------------------------------
...
toNat :: Int -> Maybe Nat
toNat n = if n < 0 then Nothing else Just (helper n)
  where helper 0 = Zero
...
fromBin :: Bin -> Int
fromBin b = fromB 0 b where
  fromB i End   = 0
  fromB i (O b) = fromB (i+1) b
  fromB i (I b) = round(2**i) + (fromB (i+1) b)

toBin :: Int -> Bin
toBin x = helper x (O End) where
  helper 0 b = b
  helper i b = helper (i-1) (inc b)
