...
longest :: Ord a => [[a]] -> [a]
longest [] = []
longest xs = foldr decide (head xs) (tail xs) where
  decide [] x           = x
  decide x []           = x
  decide (z:zs) (y:ys)  = case compare (length ys) (length zs) of
    LT -> z:zs
    GT -> y:ys
    EQ -> if z < y then z:zs else y:ys
...
incrementKey k xs = foldr (\(kk, v) acc -> (kk, if kk == k then v + 1 else v) : acc) [] xs
...
freqs xs = foldr (\x occurrences -> Map.alter (\v -> case v of
  Just v' -> Just (v' + 1)
  Nothing -> Just (1)) x occurrences) Map.empty xs
...
transfer from to amount bank = if amount < 0 then bank else
  case ((Map.member from bank), (Map.member to bank), Map.findWithDefault 0 from bank) of
    (True, True, moneyLeft) -> if moneyLeft - amount < 0 then bank else
      Map.adjust (\v -> v - amount) from $ Map.adjust (\v -> v + amount) to bank
    otherwise               -> bank
...
maxIndex arr = fst $ foldr (\(i, e) (i', e') -> if e' > e then (i', e') else (i, e) ) (head $ assocs arr) (assocs arr)
